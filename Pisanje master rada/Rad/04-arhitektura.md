...
Значајна пројектна одлука односи се на место извршавања алгоритама. Они се извршавају и на клијенту и на серверу, али са различитим наменама. На клијенту се извршавају ради приказа и сваки корак се бележи као догађај, а на серверу без бележења трага, ради брзине, и то при израчунавању референтног решења у режиму полигона, при провери кандидат мапа и при покретању мерења. Клијентска реализација оптимизована је за прегледност трага, а серверска за пропусност, зато што обради више од две хиљаде симулација у секунди.

## Јединствени интерфејс алгоритма

Централна пројектна идеја јесте потпуно раздвајање алгоритма од приказа. Приказ не зна ништа о томе који се алгоритам извршава, већ реагује искључиво на низ догађаја које алгоритам емитује, због чега додавање новог алгоритма не захтева ниједну измену у делу задуженом за приказ.

Сваки алгоритам реализује исти интерфејс, наведен у прилогу Б, који чине метода за иницијализацију, метода за корак која извршава једну логичку операцију и враћа списак насталих догађаја, метода за проверу завршетка и метода за резултат. Реч је о примени обрасца државне машине, пошто алгоритам није написан као једна петља која се извршава до краја, већ је разложен на дискретне кораке које позивалац покреће када му одговара. Тиме се анимација може паузирати између било која два корака, корисник може прећи на произвољан корак, а исти код може се покренути и без приказа.
...
Практична вредност овог решења показала се током исправљања грешака, јер свака исправка у машини претраге истовремено обухвата пет алгоритама, а разлике у понашању могу да потичу само из приоритетне функције, чиме је простор за тражење узрока грешке знатно сужен.
...
Приказ је реализован као засебна услуга која на основу текућег стања исцртава целу мрежу, а свака ћелија добија боју према улози коју у том тренутку има. Приликом реализације решена су два проблема која нису очигледна унапред. Први се односи на екране са повећаном густином тачака, на којима платно подразумевано изгледа замућено, а решење је да се физичка величина платна и цртачки контекст скалирају чиниоцем густине приказа, уз напомену да претварање положаја показивача у координате ћелије мора да користи логичке пикселе. Други се односи на променљиве димензије мреже, јер систем допушта од пет до две стотине редова и колона, па је решење да укупна површина приказа остане непроменљива, а да се величина ћелије израчуна дељењем расположиве површине бројем ћелија.
...
Серверски слој излаже програмски интерфејс подељен у седам група путања: аутентикација, мапе, покретања, полигон, вештачка интелигенција, мерење и слање слика. Свака путања која приступа корисничким подацима заштићена је посредником који проверава веб токен. Свака путања која прима идентификатор проверава да ли је он исправног облика пре него што приступи бази, а путање које примају мрежу проверавају да ли су координате полазног и циљног чвора унутар задатих димензија, што је провера чији изостанак доводи до приступа изван граница низа.
...
Аутентикација је заснована на веб токенима. Приликом пријаве сервер проверава лозинку поређењем са сачуваним сажетком добијеним функцијом bcrypt и издаје потписан токен који клијент чува локално. Уграђено је више заштитних мера. Број покушаја пријаве ограничен је на десет у периоду од петнаест минута по адреси, безбедносна заглавља поставља посредник helmet, величина тела захтева ограничена је на два мегабајта, а клијент аутоматски одјављује корисника након петнаест минута неактивности. Приступ туђим подацима спречен је провером власништва на свакој путањи, а кључ за приступ језичким моделима никада не напушта сервер. Систем подржава српски и енглески језик, као и светлу и тамну тему. Боје које се цртају на платну услуга за приказ бира према текућој теми, јер се оне не могу дефинисати као променљиве стилских листова.

Ово поглавље описало је грађу система, при чему централно место заузима јединствен интерфејс алгоритма са системом догађаја, који је омогућио да приказ остане потпуно независан од алгоритама. Наредно поглавље приказује како се ове могућности испољавају у режимима рада доступним кориснику.
