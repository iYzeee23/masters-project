  ...
  template: `
    ...
    >
      ...
      >
        <!-- Logo -->
        <div
          style="display: flex; align-items: center; justify-content: center; gap: 12px; margin-bottom: 24px; cursor: default; transition: all 0.3s;"
        ...
        >
          ...
          >
            ...
            <!-- Path -->
            <path
              d="M8 32 L8 20 L20 20 L20 8 L32 8"
              [attr.stroke]="isDark ? '#EDE0D0' : '#6E473B'"
              stroke-width="3"
          ...
          <span
            style="font-family: 'Cormorant Garamond', serif; font-size: 40px; font-weight: 750; letter-spacing: -0.01em; line-height: 1;"
        ...
        <form (ngSubmit)="onSubmit()" style="display: flex; flex-direction: column; gap: 16px;">
          ...
          @if (!isLogin) {
            <div style="display: flex; justify-content: center; margin-bottom: 16px;">
              ...
              >
                <div
                  style="width: 80px; height: 80px; border-radius: 50%; overflow: hidden; transition: all 0.3s;"
          ...
          @if (error || passwordMismatch) {
            <p style="font-size: 13px; text-align: center; color: #C4736E; margin: 0;">
...
export class AuthPageComponent implements OnDestroy {
  ...
  get passwordMismatch(): boolean {
    return (
      !this.isLogin && this.confirmPassword.length > 0 && this.password !== this.confirmPassword
    );
  }
  ...
  onSubmit(): void {
    ...
    if (this.isLogin) {
      ...
      if (this.password !== this.confirmPassword) {
        this.error = this.i18n.t('auth.passwordsMismatch');
        return;
      }
      if (this.password.length < 6) {
        this.error = this.i18n.t('auth.passwordMinLength');
        return;
      }
      this.loading = true;
      this.auth
        .register(this.username, this.email, this.password, this.firstName, this.lastName)
        .subscribe({
          next: () => {
            if (this.avatarFile) {
          ...
          error: (err) => {
            this.error = err.error?.error || this.i18n.t('auth.registrationFailed');
            this.loading = false;
          },
        });
    }
