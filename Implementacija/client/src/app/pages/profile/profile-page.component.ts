...
@Component({
  ...
  template: `
    <div
      ...
      style="max-width: 1100px; margin: 0 auto; padding: 40px 24px; display: flex; gap: 32px; min-height: calc(100vh - 80px);"
    >
      <!-- LEFT SIDEBAR -->
      <div style="width: 280px; flex-shrink: 0;">
        ...
        <div
          ...
          [style.box-shadow]="
            isDark ? '0 12px 40px rgba(0,0,0,0.3)' : '0 12px 40px rgba(0,0,0,0.08)'
          "
        ...
        <!-- User info -->
        <div
          style="margin-top: 24px; transition: transform 0.25s cubic-bezier(0.4,0,0.2,1); cursor: default;"
          onmouseover="this.style.transform='scale(1.02)'"
          onmouseout="this.style.transform='scale(1)'"
        >
          <h1
            style="font-size: 24px; font-weight: 700; letter-spacing: -0.02em; margin: 0; line-height: 1.3;"
            [style.color]="isDark ? '#EDE0D0' : '#2F241D'"
          >
            {{ user?.firstName || '' }} {{ user?.lastName || '' }}
          </h1>
          <p
            style="font-size: 18px; font-weight: 400; margin-top: 2px; margin-bottom: 0; font-style: italic;"
        ...
        <div style="margin-top: 16px; display: flex; flex-direction: column; gap: 8px;">
          <div
            style="display: flex; align-items: center; gap: 8px; transition: transform 0.2s; cursor: default; padding: 4px 0;"
            onmouseover="this.style.transform='translateX(4px)'"
            onmouseout="this.style.transform='translateX(0)'"
          >
            <svg
              style="width: 15px; height: 15px; flex-shrink: 0;"
              [style.color]="isDark ? '#655642' : '#AE9D8D'"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <span style="font-size: 13px;" [style.color]="isDark ? '#8B7460' : '#8F7C6E'">{{
              user?.email || '...'
            }}</span>
          </div>
          <div
          ...
          >
            ...
            <span style="font-size: 13px;" [style.color]="isDark ? '#8B7460' : '#8F7C6E'"
              >{{ i18n.t('profile.memberSince') }} {{ memberSince }}</span
            >
          </div>
        </div>
        ...
        <!-- Divider -->
        <div
          style="margin-top: 20px; margin-bottom: 20px; height: 1px;"
          [style.background-color]="isDark ? '#45352C' : '#D7CABC'"
        ></div>
      ...
      <div style="flex: 1; min-width: 0;">
        <!-- Statistics -->
        <div
          style="border-radius: 16px; padding: 24px; transition: all 0.3s;"
        ...
        >
          <h3
            style="font-size: 16px; font-weight: 700; margin: 0 0 16px 0;"
            [style.color]="isDark ? '#EDE0D0' : '#2F241D'"
          >
          ...
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px;">
            <div
              style="border-radius: 12px; padding: 20px 16px; text-align: center; transition: all 0.2s;"
              [style.background-color]="isDark ? '#2E241C' : '#F5EFE7'"
            ...
            >
              ...
              <div
                style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.08em; font-weight: 600; margin-top: 4px;"
                [style.color]="isDark ? '#8B7460' : '#8F7C6E'"
              >
                {{ i18n.t('profile.totalRuns') }}
              </div>
            </div>
            <div
              style="border-radius: 12px; padding: 20px 16px; text-align: center; transition: all 0.2s;"
        ...
        <!-- Leaderboard -->
        <div
          style="border-radius: 16px; padding: 24px; margin-top: 16px; transition: all 0.3s;"
          [style.background-color]="isDark ? '#3D3128' : '#FFF9F2'"
          [style.border]="isDark ? '1px solid #554536' : '1px solid #D7CABC'"
          onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 8px 30px rgba(0,0,0,0.1)'"
          onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none'"
        >
          ...
          @if (leaderboard.length === 0) {
            <div style="text-align: center; padding: 24px 0;">
              <p style="font-size: 14px; margin: 0;" [style.color]="isDark ? '#8B7460' : '#AE9D8D'">
                {{ i18n.t('profile.noEntries') }}
              </p>
            </div>
          }
          ...
          @for (entry of leaderboard; track entry; let i = $index) {
            ...
            >
              <span
                style="font-size: 14px; font-weight: 700; width: 24px; text-align: center;"
                [style.color]="
                  i === 0 ? (isDark ? '#C9A87C' : '#6E473B') : isDark ? '#8B7460' : '#AE9D8D'
                "
              >
                {{ i + 1 }}
              </span>
    ...
    @if (showZoom) {
      ...
      >
        <div
          class="animate-fade-in"
          style="width: 400px; height: 400px; border-radius: 50%; overflow: hidden; cursor: pointer;"
          [style.border]="isDark ? '4px solid #554536' : '4px solid #D7CABC'"
