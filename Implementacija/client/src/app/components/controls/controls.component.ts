  ...
  template: `
    <div class="flex flex-col gap-2">
      ...
      <div [class]="cardClass" class="animate-fade-in">
        ...
        >
          @for (algo of algorithms; track algo) {
            <option [value]="algo.value">{{ i18n.t('algo.' + algo.value) }}</option>
          }
        </select>
        <p
          [class]="
            isDark
              ? 'text-[11px] text-stone-500 mt-1.5 leading-relaxed'
              : 'text-[11px] text-stone-400 mt-1.5 leading-relaxed'
          "
        >
      ...
      <div [class]="cardClass">
        ...
        >
          <span [class]="isDark ? 'text-stone-300' : 'text-stone-600'">{{
            i18n.t('controls.tools')
          }}</span>
          <span
            [class]="isDark ? 'text-stone-600 text-xs' : 'text-stone-400 text-xs'"
            class="transition-transform duration-200"
            [style.transform]="showTools ? 'rotate(180deg)' : ''"
            >?</span
          >
        </button>
        ...
        @if (showTools) {
          <div class="mt-2 animate-slide-down">
            <div class="grid grid-cols-5 gap-1">
              @for (tool of getToolsList(); track tool) {
                <button
            ...
            @if (activeTool === 'weight') {
              <div class="mt-2 flex items-center gap-2">
                ...
                />
                <span
                  [class]="isDark ? 'text-xs text-stone-500 w-4' : 'text-xs text-stone-400 w-4'"
                  >{{ weightValue }}</span
                >
              </div>
            }
      ...
      <div [class]="cardClass">
        ...
        @if (showSettings) {
          <div class="mt-2 flex flex-col gap-2 animate-slide-down">
            <!-- Heuristic -->
            <select
            ...
            @if (isSwarmFamily()) {
              <div class="flex items-center gap-2">
                <span [class]="isDark ? 'text-xs text-stone-500' : 'text-xs text-stone-400'"
                  >w</span
                >
                <input
                  type="range"
                ...
                />
                <span
                  [class]="isDark ? 'text-xs text-stone-400 w-6' : 'text-xs text-stone-500 w-6'"
                  >{{ swarmWeight }}</span
                >
              </div>
            }
        ...
        <div [class]="cardClass" class="animate-fade-in">
          <div class="grid grid-cols-2 gap-y-1.5 text-xs">
            <span [class]="isDark ? 'text-stone-500' : 'text-stone-400'">{{
              i18n.t('metrics.expanded')
            }}</span>
            <span
              [class]="
                isDark
                  ? 'text-stone-300 text-right font-mono'
                  : 'text-stone-700 text-right font-mono'
              "
              >{{ metrics.expandedCount }}</span
            >
            <span [class]="isDark ? 'text-stone-500' : 'text-stone-400'">{{
              i18n.t('metrics.cost')
            }}</span>
          ...
          <div class="flex gap-1 mt-2">
            <button
              ...
              [class]="
                isDark
                  ? 'flex-1 text-[10px] text-stone-600 hover:text-stone-400 transition-colors'
                  : 'flex-1 text-[10px] text-stone-400 hover:text-stone-600 transition-colors'
              "
            >
              JSON ?
            </button>
