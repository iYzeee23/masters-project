...
@Component({
  ...
  template: `
    ...
    @if (isActive) {
      <div class="fixed inset-0 z-40" (click)="helper.stop()">
    ...
    @if (isActive && step) {
      ...
      >
        <div
          style="border-radius: 16px; padding: 24px;"
        ...
        >
          ...
          <!-- Title -->
          <h3
            style="font-size: 16px; font-weight: 700; margin: 0 0 8px 0;"
...
export class HelperOverlayComponent implements OnDestroy {
  ...
  tooltipStyle: { top: string; left: string; transform?: string } = {
    top: '50%',
    left: '50%',
    ...
    this.subs.push(
      ...
      this.helper.spotlightRect$.subscribe((r) => {
        if (r && r.width > 0) {
          ...
          const x1 = r.left - pad,
            y1 = r.top - pad;
          const x2 = r.left + r.width + pad,
            y2 = r.top + r.height + pad;
  ...
  private positionTooltip(): void {
    ...
    if (pos === 'bottom' && s.top + s.height + gap + tooltipH < vh) {
      const left = Math.max(
        gap,
        Math.min(s.left + s.width / 2 - tooltipW / 2, vw - tooltipW - gap),
      );
