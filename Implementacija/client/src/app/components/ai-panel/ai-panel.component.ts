    ...
    >
        ...
        <div>
          ...
          @if (keyMoments.length > 0) {
            <div class="mt-2 flex flex-col gap-2">
              @for (m of keyMoments; track m) {
                <div
                  [class]="
                    isDark
                      ? 'bg-slate-900 rounded p-2 border border-slate-700'
                      : 'bg-slate-50 rounded p-2 border border-slate-200'
                  "
                >
                  <span class="text-xs font-mono text-blue-400">Korak #{{ m.stepIndex }}</span>
                  <p
                    [class]="isDark ? 'text-xs text-slate-300 mt-1' : 'text-xs text-slate-600 mt-1'"
                  >
                    {{ m.explanation }}
                  </p>
                </div>
      ...
      @if (activeTab === 'generate') {
        <div>
          ...
          <button
            (click)="runGenerate()"
            [disabled]="loadingGenerate || !generatePrompt.trim()"
          ...
          @if (generateResult) {
            <p [class]="isDark ? 'text-xs text-slate-400 mt-2' : 'text-xs text-slate-500 mt-2'">
              {{ generateResult }}
            </p>
          }
        </div>
          ...
          @if (recommendation) {
            <div class="mt-2">
              <div
                [class]="
                  isDark
                    ? 'bg-slate-900 rounded p-2 border border-slate-700 mb-2'
                    : 'bg-slate-50 rounded p-2 border border-slate-200 mb-2'
                "
              >
                <span class="text-xs text-green-400 font-medium"
                  >✓ Najbolji: {{ recommendation.best.algorithm }}</span
                >
                <p [class]="isDark ? 'text-xs text-slate-300 mt-1' : 'text-xs text-slate-600 mt-1'">
                  {{ recommendation.best.reason }}
                </p>
                <span [class]="isDark ? 'text-xs text-slate-500' : 'text-xs text-slate-400'"
                  >Pouzdanost: {{ recommendation.best.confidence }}%</span
                >
              </div>
              <div
                [class]="
              ...
              >
                <span class="text-xs text-red-400 font-medium"
                  >✗ Najgori: {{ recommendation.worst.algorithm }}</span
                >
                <p [class]="isDark ? 'text-xs text-slate-300 mt-1' : 'text-xs text-slate-600 mt-1'">
                  {{ recommendation.worst.reason }}
                </p>
                <span [class]="isDark ? 'text-xs text-slate-500' : 'text-xs text-slate-400'"
                  >Pouzdanost: {{ recommendation.worst.confidence }}%</span
                >
              </div>
            </div>
          }
      ...
      @if (activeTab === 'generators') {
        <div>
          ...
          <div class="mt-2 flex gap-2 items-center">
            <label [class]="isDark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'"
              >Gustina:</label
            >
            <input type="range" [(ngModel)]="genDensity" [min]="10" [max]="50" class="flex-1" />
            <span [class]="isDark ? 'text-xs text-slate-300 w-8' : 'text-xs text-slate-700 w-8'"
              >{{ genDensity }}%</span
            >
          </div>
          <div class="flex gap-2 items-center">
            ...
            <button
              ...
              [class]="
                isDark
                  ? 'px-2 py-1 rounded text-xs bg-slate-700 text-slate-300 hover:bg-slate-600'
                  : 'px-2 py-1 rounded text-xs bg-slate-200 text-slate-700 hover:bg-slate-300'
              "
            >
              🎲
            </button>
